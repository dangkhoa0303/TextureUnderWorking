...
public class GLDrawer2D {
	...
	public void draw(final int tex_id, final float[] tex_matrix) {
		...
		// render on non-displayable frame buffer -- so expecting not see to the camera render here
		GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, frame_id);
	 ...
	 * @return texture ID
	 */
	public static ArrayList<Integer> initTex() {
		if (DEBUG) Log.v(TAG, "initTex:");
		final int[] tex = new int[1];
		final int[] fbo = new int[1];
		final int[] renders = new int[1];

		GLES20.glActiveTexture(GLES20.GL_TEXTURE0);

		/* My code */
		// generate framebuffer object -- don't use the the window-system-provided framebuffer
		GLES20.glGenFramebuffers(1, fbo, 0);
		GLES20.glBindFramebuffer(GLES11Ext.GL_TEXTURE_EXTERNAL_OES, fbo[0]);

		GLES20.glGenRenderbuffers(1, renders, 0);
		GLES20.glBindRenderbuffer(GLES20.GL_RENDERBUFFER, renders[0]);
		/* My code */
		...
		/* My code */
		GLES20.glFramebufferTexture2D(GLES20.GL_FRAMEBUFFER, GLES20.GL_COLOR_ATTACHMENT0, GLES11Ext.GL_TEXTURE_EXTERNAL_OES, tex[0], 0);
		GLES20.glFramebufferRenderbuffer(GLES20.GL_FRAMEBUFFER, GLES20.GL_DEPTH_ATTACHMENT, GLES20.GL_RENDERBUFFER, renders[0]);
		/* My code */
